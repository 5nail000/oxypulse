...
Sample g_samples[TREND_CAPACITY];
size_t g_head = 0;
size_t g_count = 0;
uint32_t g_t0_ms = 0;
...
void trendBufferInit() {
    ...
    loadMask();
    logPrintf("Trends: RAM ring %u x 1s (~30 мин), mask=0x%lx, heap=%u",
...
void trendBufferWriteJson(TrendJsonSink sink, size_t offset, size_t limit) {
    ...
    const size_t total = (g_mask == 0) ? 0 : g_count;
    size_t start = offset;
    if (start > total) {
        start = total;
    }
