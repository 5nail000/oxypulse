...
bool parseHrMeasurement(const uint8_t *data, size_t len, uint16_t *bpm, bool *contact,
                        uint16_t *rr_list, size_t rr_max, size_t *rr_count) {
    ...
    if ((flags & 0x10U) != 0) {
        while (idx + 1 < len) {
            ...
            idx += 2;
            const uint16_t rr_ms =
                static_cast<uint16_t>((static_cast<uint32_t>(raw) * 1000U) / 1024U);
            if (rr_list != nullptr && n < rr_max) {
                rr_list[n] = rr_ms;
                n += 1;
            }
        }
...
void coospoNotifyCb(NimBLERemoteCharacteristic *characteristic, uint8_t *data, size_t length, bool is_notify) {
    ...
    (void)is_notify;
    uint16_t bpm = 0;
    uint16_t rr_list[HRV_RR_PER_NOTIFY_MAX];
    size_t rr_n = 0;
    bool contact = false;
    if (!parseHrMeasurement(data, length, &bpm, &contact, rr_list, HRV_RR_PER_NOTIFY_MAX, &rr_n)) {
        return;
    }
    const uint16_t last_rr = rr_n > 0 ? rr_list[rr_n - 1] : 0;
    if (!tryLock()) {
        return;
    }
    ...
    if (contact && last_rr >= 250 && last_rr <= 2000) {
        g_coospo.rr_ms = last_rr;
    } else {
        g_coospo.rr_ms = 0;
    }
    ...
    unlock();
    if (contact) {
        for (size_t i = 0; i < rr_n; i += 1) {
            uartTxSendRr(rr_list[i]);
        }
    }
}
...
void bleSensorsInit() {
    ...
    logPrintf("BLE: NimBLE central ready, heap=%u", ESP.getFreeHeap());
